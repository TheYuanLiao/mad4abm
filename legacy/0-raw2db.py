...
if __name__ == '__main__':
    ...
    if flag == 1:
        ...
        for file, tb_name in zip(file_list, ('vgr_stops_2019_yl',)): # , 'vgr_point_types_2019'
            print("Loading", file)
            preprocess.raw2chunk2db(file, user, password, port, db_name,
                                    table_name=tb_name, schema_name='public', folder_under_db='original_input_data_yl')
            print(file, "done.")
        # 1.2 Load inferred home data
        print("Loading homes...")
        df_home = pd.read_csv(os.path.join(ROOT_dir, "dbs/original_input_data_yl/vgr_homes_2019.csv"))
        preprocess.dump2db_df(df_home, user, password, port, db_name,
                              table_name='vgr_homes_2019_yl', schema_name='public')
        print("...done.")
    if flag == 2:
        # 2. Inject zones
        print("Loading zones...")
